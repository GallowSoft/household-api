...
export class ItemPricesResolver {
  ...
  async itemPrice(@Args('id', { type: () => ID }) id: string): Promise<ItemPrice | null> {
    const supabase = this.supabaseService.getClient();
  ...
  @Query(() => [ItemPrice])
  async currentPricesForItem(@Args('inventoryItemId', { type: () => ID }) inventoryItemId: string): Promise<ItemPrice[]> {
    const supabase = this.supabaseService.getClient();
    ...
    if (error) {
      throw new Error(`Failed to fetch current prices for item: ${error.message}`);
    }
  ...
  async cheapestPricesForShoppingList(): Promise<ItemPrice[]> {
    ...
    if (error) {
      throw new Error(`Failed to fetch shopping list for price comparison: ${error.message}`);
    }
    ...
    for (const item of data) {
      ...
      if (priceError) {
        console.error(`Failed to fetch prices for item ${item.inventory_item_id}:`, priceError);
        continue;
      }
      ...
      if (prices && prices.length > 0) {
        cheapestPrices.push({
          ...prices[0],
          inventoryItem: prices[0].inventory_item,
